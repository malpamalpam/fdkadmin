...
import {
  ...
  PZK_CLIENT_TYPE_LABELS, getAmountColor, canEditModule, FieldColor,
} from "@/lib/pzk-types";
      ...
      <div className="flex items-start justify-between mb-4 gap-4">
        <div>
          ...
          <div className="flex flex-wrap gap-3 text-sm text-gray-500 mt-1">
            ...
            {c.cooperationEndsAt && <span>Koniec: <strong>{new Date(c.cooperationEndsAt).toLocaleDateString("pl-PL")}</strong></span>}
            {c.withdrawnFromNotice && <span className="text-orange-600 font-medium">Rezygnacja z wypowiedzenia</span>}
          </div>
        </div>
        <div className="flex-shrink-0">
          {!c.withdrawnFromNotice && (
            <button onClick={() => setSendModal(true)} className={`px-4 py-2 rounded-lg text-sm font-medium ${c.emailInitialSent ? "bg-gray-100 text-gray-600 hover:bg-gray-200" : "bg-green-600 text-white hover:bg-green-700"}`}>
